...
@StarAgent.register("ace_adaptation_react_bank")
class SimplifiedReActStarAgent(StarAgent):
    ...
    def next_execution_inputs_and_cost(
        ...
        output = self.generator_model.generate(messages=messages, max_tokens=self.max_output_tokens)
    ...
    def solve_task_wo_gt(self, task_id: str, experiment_name: str | None = None):
        ...
        self.store_to_bank(task_success)
    ...
    def _exceeds_context_limit(self, pre_messages: list[dict], post_messages: list[dict]) -> bool:
    ...
    def trimmed_messages(self) -> list[dict]:
        ...
        observation_index = 0
        while self._exceeds_context_limit(pre_messages, post_messages) or len(output_str) > self.max_output_length:
            found_block = False
