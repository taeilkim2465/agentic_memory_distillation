...
VLLM_URL="http://localhost:8441/v1"
...
echo "[SASMLlama31_8B] VLLM_URL=$VLLM_URL  SASM_STORE=$SASM_STORE  TOP_K=$SASM_TOP_K"

if [[ ! -f "$SASM_STORE" ]]; then
  echo "ERROR: SASM store not found: $SASM_STORE"
  exit 1
fi

cd "${TOOLSANDBOX_ROOT:?Set TOOLSANDBOX_ROOT to your ToolSandbox repo path}" && tool_sandbox \
  --agent SASMLlama31_8B \
  --user GPT_5_Mini \
  --vllm-url "$VLLM_URL" \
  --sasm_store_path "$SASM_STORE" \
  --sasm_top_k "$SASM_TOP_K" \
  -p "$PARALLEL" \
  -o "$OUTPUT_DIR" \
  --scenarios \
