...
VLLM_URL="http://localhost:8771/v1"
MEMORY_DIR="memories/teacher_GPT_5_Mini_user_GPT_5_Mini_0520_1133"
PARALLEL=1
OUTPUT_DIR="data"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --memory_dir) MEMORY_DIR="$2"; shift 2 ;;
...
echo "[MemoryQwen3_8B_WF_FN_ST] VLLM_URL=$VLLM_URL  MEMORY_DIR=$MEMORY_DIR"

cd "${TOOLSANDBOX_ROOT:?Set TOOLSANDBOX_ROOT to your ToolSandbox repo path}" && tool_sandbox \
  --agent MemoryQwen3_8B_WF_FN_ST \
  --user GPT_5_Mini \
  --vllm-url "$VLLM_URL" \
